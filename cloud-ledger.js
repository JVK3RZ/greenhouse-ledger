(function(){
  ...
  function renderTeam(){
    ...
    function invitationDeliveryMessage(error){
      ...
      if(/can only send testing emails|verify a domain/i.test(detail)) return 'Email could not be delivered because a sending domain has not been verified yet. Try sending the activation email again after email delivery is configured.';
      return 'Email could not be delivered. Try sending the activation email again later.';
    }
  ...
  async function copyInvite(code){const url=new URL(location.href);url.search='';url.hash='';url.searchParams.set('invite',code);await navigator.clipboard.writeText(url.toString());showToast('Invitation link copied');}
  async function sendInvite(id,refreshAfter=true){if(LedgerAuth.getContext().isDemo)return showToast('Invitation email is disabled in demo mode. Copy the private link instead.');const {data:{session}}=await client().auth.getSession();const response=await fetch(`${window.GREENHOUSE_SUPABASE.url}/functions/v1/send-organization-invitation`,{method:'POST',headers:{'Content-Type':'application/json','apikey':window.GREENHOUSE_SUPABASE.publishableKey,'Authorization':`Bearer ${session.access_token}`},body:JSON.stringify({invitation_id:id})});const payload=await response.json().catch(()=>({}));if(!response.ok){showToast(payload.error||'Activation email is not configured. Contact the platform administrator.');if(refreshAfter)await refresh();return;}await refresh('Invitation email sent');}
