(function(){
  ...
  function renderTeam(){
    ...
    const invitations=data.invitations.map(i=>{
      ...
      return `<div class="card invite-card"><div class="card-top"><div><strong>${esc(i.email)}</strong><div class="plant-species">${esc(label(i.role))} · created ${new Date(i.created_at).toLocaleDateString()} · expires ${new Date(i.expires_at).toLocaleDateString()}</div></div><span class="status-badge status-${state}">${esc(label(state))}</span></div>${i.delivery_error?`<p class="report-note">Email delivery: ${esc(i.delivery_error)}</p>`:''}<div class="card-actions">${active?`<button class="btn small" onclick="CloudLedger.copyInvite('${i.code}')">Copy link</button><button class="btn primary small" onclick="CloudLedger.sendInvite('${i.id}')">${i.sent_at?'Resend email':'Send email'}</button><button class="btn ghost small" onclick="CloudLedger.revokeInvite('${i.id}')">Revoke</button>`:''}${state==='expired'?`<button class="btn small" onclick="CloudLedger.replaceInvite('${i.id}')">Create replacement</button>`:''}</div></div>`;
    }).join('');
